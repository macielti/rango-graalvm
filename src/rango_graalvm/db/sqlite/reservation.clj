...
(s/defn insert! :- models.reservation/Reservation
  [{:reservation/keys [id student-id menu-id created-at]} :- models.reservation/Reservation
   database-conn]
  (-> (jdbc/execute! database-conn ["INSERT INTO reservations (id, student_id, menu_id, created_at) VALUES (?, ?, ?, ?) returning *"
...
(s/defn by-menu :- [models.reservation/Reservation]
  [menu-id :- s/Uuid
   database-conn]
  (some->> (jdbc/execute! database-conn ["SELECT * FROM reservations WHERE menu_id = ?" (str menu-id)])
           (map adapters.reservation/sqlite->internal)))

(s/defn lookup :- (s/maybe models.reservation/Reservation)
  [reservation-id :- s/Uuid
   database-conn]
  (some-> (jdbc/execute! database-conn ["SELECT * FROM reservations WHERE id = ?" (str reservation-id)])
          first
          adapters.reservation/sqlite->internal))
...
(s/defn lookup-by-student-and-menu :- (s/maybe models.reservation/Reservation)
  [student-id :- s/Uuid
   menu-id :- s/Uuid
   database-conn]
  (some-> (jdbc/execute! database-conn ["SELECT * FROM reservations WHERE menu_id = ? AND student_id = ?"
...
(s/defn retract!
  [reservation-id :- s/Uuid
   database-conn]
  (jdbc/execute! database-conn ["DELETE FROM reservations WHERE id = ?" (str reservation-id)]))

(s/defn fetch-student-reservation-by-menu :- (s/maybe models.reservation/Reservation)
  [student-id :- s/Uuid
   menu-id :- s/Uuid
   database-conn]
  (some-> (jdbc/execute! database-conn ["SELECT *
                                         FROM reservations
                                         WHERE
