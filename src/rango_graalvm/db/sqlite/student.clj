...
(s/defn insert! :- models.student/Student
  ...
  (with-open [conn (jdbc/get-connection database)]
    (-> (jdbc/execute! conn ["INSERT INTO students (id, code, name, class, created_at) VALUES (?, ?, ?, ?, ?) returning *"
...
(s/defn lookup-by-code :- (s/maybe models.student/Student)
  [code :- s/Str
   database]
  (with-open [conn (jdbc/get-connection database)]
    (some-> (jdbc/execute! conn ["SELECT * FROM students WHERE code = ?" code])
            first
            adapters.student/sqlite->internal)))
...
(s/defn all :- [models.student/Student]
  [database]
  (with-open [conn (jdbc/get-connection database)]
    (some->> (jdbc/execute! conn ["SELECT * FROM students"])
...
(s/defn by-menu-reservation :- [models.student/Student]
  [menu-id :- s/Uuid
   database]
  (with-open [conn (jdbc/get-connection database)]
    (some->> (jdbc/execute! conn ["SELECT students.*
                                   FROM students
...
(s/defn retract!
  [student-id :- s/Uuid
   database]
  (with-open [conn (jdbc/get-connection database)]
    (jdbc/execute! conn ["DELETE FROM students WHERE id = ?" (str student-id)])))
